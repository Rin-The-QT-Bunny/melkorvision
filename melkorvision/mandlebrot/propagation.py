...
class GraphPropagation(nn.Module):
    ...
    def preprocess_inputs(self,h0,adj,activated):
        ...
        # create excitory and inhibitory affinities, followed by thresholding
    ...
    def forward(self,h0,adj,activated = None):
        ...
        for itr in range(self.num_iters):
            h,activated,running_activated = self.propagate(h,adj_e,adj_i,activated,running_activated,sample_mask,itr)
    ...
    def propagate(self,h,adj_e,adj_i,activated,running_activated,sample_mask,iter):
        ...
        if not self.activation_converge:
            if isinstance(adj_e,SparseTensor):
                ...
                adj_i = adj_i.mul(activated.flattent()[None])
    ...
    def _threshold_sparse_tensor(x,thresh):
        ...
        output = SparseTensor(row=row[valid],col=col[valid],value=value[valid],sparse_size=sparse_size)
        return output
    
    @staticmethod
    def _projection(v,u,eps=1e-12):
        u_norm = torch.sum(u*u,-1,keepdim=True)
        dot_prod = torch.sum(v * u,-1,keepdim=True)
